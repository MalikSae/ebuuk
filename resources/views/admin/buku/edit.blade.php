...
<div class="mb-5 text-sm text-slate-400">
    <a href="{{ route('admin.buku.index') }}" class="text-teal-600 hover:text-teal-700">Buku</a> › Edit
</div>
    ...
    <form action="{{ route('admin.buku.update', $buku->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <div class="mb-4">
                    ...
                    <input type="text" name="judul" value="{{ old('judul', $buku->judul) }}" required placeholder="Masukkan judul buku"
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="penulis" value="{{ old('penulis', $buku->penulis) }}" placeholder="Nama penulis"
                           class="w-full border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent placeholder:text-slate-400">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="halaman" value="{{ old('halaman', $buku->halaman) }}" min="1" placeholder="Contoh: 250"
                           class="w-full border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent placeholder:text-slate-400">
            ...
            <div>
                <div class="mb-4">
                    ...
                    <div class="flex items-start gap-4">
                        <img id="cover-preview" src="{{ $buku->cover ? asset('images/covers/' . $buku->cover) : asset('images/cover-default.svg') }}"
                ...
                <div class="mb-4">
                    ...
                    <div id="pdf-drop-area" class="border-2 border-dashed border-slate-200 rounded-xl p-6 text-center hover:border-teal-400 transition-colors cursor-pointer bg-slate-50">
                        ...
                        <p class="text-sm text-slate-500 mt-2">Klik untuk upload PDF baru (opsional)</p>
                    ...
                    <p class="text-xs text-slate-400 mt-1.5">File PDF saat ini: {{ $buku->file_pdf }}</p>
                    <input type="file" name="file_pdf" id="file_pdf" accept=".pdf" hidden>
        ...
        <div class="mt-4 mb-6">
            ...
            <textarea name="deskripsi" rows="4" placeholder="Tulis sinopsis atau deskripsi singkat buku..."
                      class="w-full border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent placeholder:text-slate-400">{{ old('deskripsi', $buku->deskripsi) }}</textarea>
